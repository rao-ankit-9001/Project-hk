...
const allowedOrigins = [
  "https://hamari-khani.netlify.app", 
  "http://localhost:5173"
]; 
...
app.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    res.json({ message: "File uploaded & saved to DB successfully!" });
  } catch (err) {
...
// Mount route files (upload, auth)
try {
  const uploadRoutes = require("./routes/upload");
  app.use("/api", uploadRoutes);
} catch (e) {
  console.warn("Could not mount upload routes:", e.message);
}
try {
  const authRoutes = require("./routes/auth");
  app.use("/api/auth", authRoutes);
} catch (e) {
  console.warn("Could not mount auth routes:", e.message);
}
    ...
    if (dbState === 1) {
      ...
      try {
        ...
        checkpoints.push({
          step: "collections",
          status: "ok",
          detail: collections.map((c) => c.name),
        });
      } catch (colErr) {
        checkpoints.push({
          step: "collections",
...
setInterval(() => {
  axios
    .get(`https://project-hk.onrender.com/health`) // local dev ke liye
